...
for ((book = 0; book < book_max; book++)); do
    ...
    # echo -e "${navigation}\n# ${long_title}\n\n## Contents\n" >>"${outline_dir}/${standard_abbreviation}.md"
    echo -e "# ${long_title}\n\n## Contents\n" >>"${outline_dir}/${standard_abbreviation}.md"
    # Create an overview file for each book of the Bible:
    echo -e "[[${standard_abbreviation}|${long_title}]]" >>"${outline_dir}/${standard_genre}.md"

    for ((chapter = 1; chapter <= chapter_max; chapter++)); do
        if [[ $chapter -gt 1 ]]; then
            echo -en "[[${standard_abbreviation}${chapter}|]]" >>"${outline_dir}/${standard_abbreviation}.md"
        else
            echo -en "[[${standard_abbreviation}${chapter}|Start reading (${translation})]]" >>"${outline_dir}/${standard_abbreviation}.md"
        fi
    done
done
